...
function circleClock() {
    ...
    let wCtx = document.getElementById("canvas").getAttribute("width");
    let hCtx = document.getElementById("canvas").getAttribute("height");


    ctx.save();
    ctx.clearRect(0, 0, wCtx, hCtx);
    ctx.translate(95, 95);
    ctx.scale(0.5, 0.5);
    ctx.rotate(-Math.PI / 2);
    ctx.strokeStyle = "#004383";
        // "green";
    // ctx.fillStyle = "green";
    ctx.lineWidth = 10;
    ctx.lineCap = "round";
    ...
    ctx.strokeStyle = "#8c00ff";
    ctx.fillStyle = "#8600d4";
    ctx.lineWidth = 6;
    ctx.beginPath();
    ctx.moveTo(-30, 0);
    ctx.lineTo(83, 0);
    ...
    // window.requestAnimationFrame(circleClock);
}
